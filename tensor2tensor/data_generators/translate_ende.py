...
class TranslateEndeWmtBpe32k(translate.TranslateProblem):
  ...
  def get_or_create_vocab(self, data_dir, tmp_dir, force_get=False):
    ...
    return text_encoder.TokenTextEncoder(vocab_filename, replace_oov="UNK") # 给定词表文件名，构造词表类
...
class TranslateEndeWmt8k(translate.TranslateProblem):
  ...
  def source_data_files(self, dataset_split): # 给定split_name, 返回数据源信息（在这个problem上是下载网址，和train和dev的文件名）
    train = dataset_split == problem.DatasetSplit.TRAIN
