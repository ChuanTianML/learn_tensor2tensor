...
def _data_filenames(output_name, output_dir, num_shards):
  # 生成num_shards个分片文件的文件名（已经拼接了输出路径）
  return [
...
def sharded_name(base_name, shard, total_shards): # 生成一个分片文件的文件名
  return "%s-%.5d-of-%.5d" % (base_name, shard, total_shards)


def shard_filepath(fname, num_shards):  # 生成num_shards个分片文件的文件名
  return [
...
def generate_files(generator, output_filenames, # 这个函数负责将生成器中的样本以TFRecord的格式输出
  ...
  for case in generator:
    ...
    if max_cases and counter > max_cases:
      break
    example = to_example(case) # 返回tf.train.Example
