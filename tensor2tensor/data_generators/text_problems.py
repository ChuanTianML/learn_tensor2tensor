...
class Text2TextProblem(problem.Problem):
  ...
  def get_or_create_vocab(self, data_dir, tmp_dir, force_get=False):
    if self.vocab_type == VocabType.CHARACTER: # 字符级别的词表
      encoder = text_encoder.ByteTextEncoder()
    elif self.vocab_type == VocabType.SUBWORD: # 词素级别的词表
      if force_get:
    ...
    elif self.vocab_type == VocabType.TOKEN: # token级别的词表
      vocab_filename = os.path.join(data_dir, self.vocab_filename) # 先准备好词表文件的文件名
  ...
  def generate_encoded_samples(self, data_dir, tmp_dir, dataset_split):
    generator = self.generate_samples(data_dir, tmp_dir, dataset_split) # 给定split_name，返回一个input，output映射
    encoder = self.get_or_create_vocab(data_dir, tmp_dir)
    return text2text_generate_encoded(generator, encoder, has_inputs=self.has_inputs) # 将所有的文本样本转为id样本，返回的依然是一个input，output映射，只不过文本换成了id
  ...
  def generate_data(self, data_dir, tmp_dir, task_id=-1): # 生成数据，按照TFRcord的格式输出到tmp_dir
    ...
    split_paths = [(split["split"], filepath_fns[split["split"]](data_dir, split["shards"], shuffled=False)) for split in self.dataset_splits]
    ...
    all_paths = [] # 目前有两个split，每个split有num_shards个文件名，这个all_paths负责把所有文件名打包到一个list里面
...
def text2text_txt_iterator(source_txt_path, target_txt_path): # 给定两个语言的文件，返回一个input，output映射
...
def text2text_generate_encoded(sample_generator, # 这个函数的作用是，将所有的文本样本，转为id样本
                               vocab,
                               targets_vocab=None,
                               has_inputs=True):
  """Encode Text2Text samples from the generator with the vocab."""
  targets_vocab = targets_vocab or vocab # 几个意思？ targets_vocab如果为None就用vocab，即共用一个词表？？？
  for sample in sample_generator: # 对每个样本，即<input, output>对儿
    if has_inputs:
