...
class Problem(object):
  ...
  def training_filepaths(self, data_dir, num_shards, shuffled): # 生成num_shards个分片文件的文件名（已经拼接了输出路径）
    file_basename = self.dataset_filename() # 获得任务名
    if not shuffled:
      file_basename += generator_utils.UNSHUFFLED_SUFFIX # 在任务名后面接上"-unshuffled"以表示没有shuffle
