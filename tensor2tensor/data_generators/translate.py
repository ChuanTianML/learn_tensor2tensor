...
class TranslateProblem(text_problems.Text2TextProblem):
  ...
  def source_data_files(self, dataset_split): # 给定split_name, 返回数据源信息（在这个ende problem上是下载网址，和train和dev的文件名）
  ...
  def generate_samples(self, data_dir, tmp_dir, dataset_split):
    datasets = self.source_data_files(dataset_split) # 给定split_name, 返回数据源信息（在这个ende problem上是下载网址，和train和dev的文件名）
    tag = "train" if dataset_split == problem.DatasetSplit.TRAIN else "dev" # 获得split_tag, "train/dev"
    data_path = compile_data(tmp_dir, datasets, "%s-compiled-%s" % (self.name, tag)) # 给定数据源信息，将数据下载并预处理，输出到tmp_dir文件夹底下，按照给定的文件名进行命名
    return text_problems.text2text_txt_iterator(data_path + ".lang1", data_path + ".lang2") # 给定两个语言的文件，返回一个input，output映射
...
def _preprocess_sgm(line, is_sgm): # 删掉辅助的block，只留下文本
