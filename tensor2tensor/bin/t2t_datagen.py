...
_SUPPORTED_PROBLEM_GENERATORS = { # 这是任务名字与数据生成器的对应？
...
def generate_data_for_registered_problem(problem_name): # 给已注册的任务生成数据
  """Generate data for a registered problem."""
  tf.logging.info("Generating data for %s.", problem_name)
  if FLAGS.num_shards: # 已注册的任务不能分片？？？
    raise ValueError("--num_shards should not be set for registered Problem.")
  problem = registry.problem(problem_name) # 根据名字找任务？目前还不知道返回的是什么类型
  task_id = None if FLAGS.task_id < 0 else FLAGS.task_id
  data_dir = os.path.expanduser(FLAGS.data_dir)
  tmp_dir = os.path.expanduser(FLAGS.tmp_dir)
  if task_id is None and problem.multiprocess_generate: # 多线程方式，没看
    ...
    problem.generate_data(data_dir, tmp_dir, task_id) # 生成数据
