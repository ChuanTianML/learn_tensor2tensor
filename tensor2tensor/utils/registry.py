...
def problem(name):
  ...
  def parse_problem_name(problem_name):
    ...
    Args:
      problem_name: str, problem name, possibly with suffixes. # 任务的名字

    Returns:
      base_name: A string with the base problem name. # 基础任务名？？？
      was_reversed: A boolean. # 是否翻转，例如翻译任务中，中译英和英译中
      was_copy: A boolean. # 是否复制 ？？？
    """
    # Recursively strip tags until we reach a base name.
    if problem_name.endswith("_rev"): # 是否翻转和是否复制，从任务后缀来判断
  ...
  if base_name not in _PROBLEMS: # 指定的任务必须在已知任务列表中，否则报错
    all_problem_names = list_problems()
    error_lines = ["%s not in the set of supported problems:" % base_name
                  ] + all_problem_names
    error_msg = "\n  * ".join(error_lines)
    raise LookupError(error_msg)
  return _PROBLEMS[base_name](was_reversed, was_copy) # 这是什么语法？？？返回的是什么类型
